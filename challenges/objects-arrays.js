...
universities.sort(function(alpha, beta) {
  if (alpha.university > beta.university) {
    return 1;
  }
  if (alpha.university < beta.university) {
    return -1;
...
for(let i = 0; i < graduates.length; i++) {
  let contacts = {};
  contacts.first_name = graduates[i].first_name;
  contacts.email = graduates[i].email;
  contactInfo.push(`${contacts.first_name} ${contacts.email}`);
  contacts = {};
}
...
const unisWithUni = [];
for (let i = 0; i < graduates.length; i++) {
  let univsUni = graduates[i].university;
  if (univsUni.includes("Uni")) {
    unisWithUni.push(univsUni);
  };
};
...
const lowCaseAnimalNames = zooAnimals.map(function(animal) {
  animal.animal_name = animal.animal_name.toLowerCase();
...
*/
const populationTotal = zooAnimals.reduce((popAcc, popCur) => popAcc + popCur.population, 0);
